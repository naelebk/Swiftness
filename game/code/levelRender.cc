...
namespace swiftness{
    ...
    void LevelRender::renderLayer(LayerEntity &layerEntity, gf::RenderWindow &window, LayerName layerName, std::string gravity)
    {
        ...
        if (layer->visible == true){
            if ((layerName == LayerName::Gw_blue && gravity == "down") || (layerName == LayerName::Gw_rose && gravity == "up") || (layerName == LayerName::Gw_green && gravity == "left") || (layerName == LayerName::Gw_orange && gravity == "right")){
                layer->opacity == 0.6;
            }
