...
namespace swiftness
{
    ...
    void SelectLevel::doHandleActions(gf::Window &window)
    {
        if (!window.isOpen())
            return;
        if (up.isActive() || left.isActive())
            widgets.selectPreviousWidget();
        if (down.isActive() || right.isActive())
    ...
    void SelectLevel::doRender(gf::RenderTarget &target, const gf::RenderStates &states)
    {
        ...
        float yPos = coords.getRelativePoint(bg_size + 0.275f).y;
        ...
        yPos = coords.getRelativePoint(bg_size + 0.55f).y;
        ...
        yPos = coords.getRelativePoint(bg_size + 0.775f).y;
